...
class Model_INCEPTION_ORIG(BaseModel):
...
class Model_ENCODER_ORIG(BaseModel):
    def __init__(self, input_shape, nb_classes):
        self.callbacks = []
        self.batch_size = 12
        # self.nb_epochs = 100
...
class HeartNet2D_ORIG(nn.Module):
    def __init__(self, input_shape, num_classes=7):
        super(HeartNet2D_ORIG, self).__init__()
...
class ZolotyhNet_ORIG(nn.Module):
...
class Model_ZolotyhNet_ORIG(BaseModel):
    def __init__(self, input_shape, nb_classes):
        self.batch_size = 128
        # self.batch_size = 16
        self.nb_epochs = 1000
    ...
    def build_model(self, input_shape, nb_classes):
        inner_model = ZolotyhNet_ORIG(input_shape[::-1], num_classes=nb_classes)
        optimizer = torch.optim.Adam(inner_model.parameters(), lr=1e-3)
        model = LitModule(inner_model, optimizer)
        return model


class Model_HeartNet2D_ORIG(BaseModel):
    def __init__(self, input_shape, nb_classes):
        self.batch_size = 128
        # self.batch_size = 16
        self.nb_epochs = 300
    ...
    def build_model(self, input_shape, nb_classes):
        inner_model = HeartNet2D_ORIG(input_shape[::-1], num_classes=nb_classes)
        optimizer = torch.optim.Adam(inner_model.parameters(), lr=1e-3)
        model = LitModule(inner_model, optimizer)
        return model
