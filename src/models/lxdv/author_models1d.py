...
class BasicBlockHeartNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            ...
            identity = F.max_pool1d(identity, self.stride, ceil_mode=True)
        else:
            identity = F.max_pool1d(identity, 1, ceil_mode=True)
...
class HeartNet1D(nn.Module):
    def __init__(
        ...
        super(HeartNet1D, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm1d
        ...
        self.conv1 = conv_block(channels, self.inplanes, stride=1)
